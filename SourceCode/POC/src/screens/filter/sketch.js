  ...
  useEffect(() => {
    try {
      const validatedData = questionsData.map((item, index) => {
        if (!item.clNo || !item.q || !item.date || !item.status || !item.HR_SK) {
  ...
  const getFilteredQuestions = () => {
    try {
      ...
      return questions.filter(question => (
        ...
        question.Sketch.toLowerCase().includes(query)
      ));
    } catch (error) {
            ...
            filteredQuestions.map((question, index) => (
              ...
              >
                ...
                <div className="question-meta">
                  ...
                  <div className="sketch-badge highlight">{question.Sketch}</div>
                </div>
              </div>
            ))
          ) : (
