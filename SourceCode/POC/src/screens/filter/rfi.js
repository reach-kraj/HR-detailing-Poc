...
const RFI = () => {
  ...
  // Filter questions by HR_RFI and search query
  const getFilteredQuestions = () => {
    try {
      // Filter for questions with a non-empty HR_RFI
      const validQuestions = questions.filter(
        (item) => item.HR_RFI && item.HR_RFI.trim()
      );
      ...
      const filtered = validQuestions.filter(
        (question) =>
          ...
          question.HR_RFI.toLowerCase().includes(query)
      );
