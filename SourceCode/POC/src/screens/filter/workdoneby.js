...
const WorkDoneBy = () => {
  ...
  useEffect(() => {
    try {
      const validatedData = questionsData
        .filter(item => item.Work_Done_By && item.Work_Done_By.trim() !== '') // Ensure Work_Done_By is not empty
        .map(item => ({
          clNo: item.clNo.trim(),
          q: item.q.trim(),
          date: item.date.trim(),
          status: item.status.toLowerCase().trim(),
          workDoneBy: item.Work_Done_By.trim(),
        }));
  ...
  const getFilteredQuestions = () => {
    ...
    return questions.filter(question => 
      ...
      question.workDoneBy.toLowerCase().includes(query)
    );
  };
            ...
            filteredQuestions.map((question, index) => (
              ...
              >
                ...
                <div className="question-meta">
                  ...
                  <div className="design-reference-badge highlight">{question.workDoneBy}</div>
                </div>
              </div>
            ))
          ) : (
