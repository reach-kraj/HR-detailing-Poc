  ...
  useEffect(() => {
    try {
      const filteredData = questionsData
        .filter(item => item.status && item.status.toLowerCase().trim() === 'closed') // Only show "closed" status
        .map(item => ({
          clNo: item.clNo.trim(),
          q: item.q.trim(),
          date: item.date.trim(),
        ...
        <div className="questions-list">
          {filteredQuestions.length > 0 ? (
            ...
            <p className="no-results">
              {searchQuery ? `No results found for "${searchQuery}"` : 'No closed questions available'}
            </p>
          )}
        </div>
      </div>
