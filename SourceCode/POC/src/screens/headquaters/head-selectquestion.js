...
const SelectQuestion = () => {
  ...
  // Filter only open questions and apply search
  const getFilteredQuestions = () => {
    try {
      // Filter for open status first
      const openQuestions = questions.filter(
        (item) => item.status && item.status.toLowerCase().trim() === "open"
      );

      if (!searchQuery.trim()) return getSortedQuestions(openQuestions);

      const query = searchQuery.toLowerCase().trim();
      const filtered = openQuestions.filter(
        (question) =>
